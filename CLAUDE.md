...
- `repomix-output.xml` at the repo root is a stale packed snapshot of the codebase (for LLM context). Treat it as read-only reference; it may not reflect current source.
- `reference/` contains compressed XML dumps of the upstream `prosemirror-kotlin` and `richeditor-compose` sources (read-only reference). The richeditor-compose `RichTextState` is a flat-model implementation — port its API surface and UX, not its internals.
