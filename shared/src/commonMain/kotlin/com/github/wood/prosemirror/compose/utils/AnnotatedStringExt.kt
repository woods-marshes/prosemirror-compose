...
internal fun AnnotatedString.Builder.appendProseMirrorDoc(
    ...
    fun compileNode(
        ...
        if (isBlock && !isDoc) {
            // 块起始开销
            pmCursor += 1
            // 块分隔符（合成 "\n"）：只加在"非父的首个子块 且 非列表项"的块前——
            // 列表项由 marker 承担分隔，item 内首块由 marker 对齐。
            val isListItem = node.type.name == "list_item"
            if (length > 0 && !isFirstChildOfParent && !isListItem &&
                toAnnotatedString().text.last() != '\n'
            ) {
        ...
        if (node.isLeaf && !node.isText) {
            ...
            mapBuilder.registerRange(pmCursor, pmCursor + node.nodeSize, flatStart, length)
            pmCursor += node.nodeSize
        } else {
